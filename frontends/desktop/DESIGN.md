...
详细 microcopy 对照表见 `.trellis/spec/desktop/frontend/microcopy.md`。
...
- Thread 系统：使用 `data-slot` 属性选择器（如 `[data-slot="user-bubble"]`）——源自 Hermes 设计系统的 slot 模型，保证组件可组合而不依赖 DOM 层级
...
| 文档 | 位置 | 内容 |
|------|------|------|
| Microcopy 对照表 | `.trellis/spec/desktop/frontend/microcopy.md` | 数据维护区域的完整三层文案 + 错误路由 |
| 本地仓库连接契约 | `.trellis/spec/desktop/frontend/local-repo-connection.md` | 连接流程状态机 + Rust 命令签名 + 验证规则 |
| 记忆导入设计 | `.trellis/spec/desktop/frontend/memory-import.md` | 导入流程 + 后端 API 契约 |
