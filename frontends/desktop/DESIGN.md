...
- “运行时管理”是入口与面板名称，“本地运行环境”是被管理对象，“运行组件”是表格中的具体对象
- “运行状态”页默认显示产品名 + 一行常驻功能说明；Tooltip 提供更完整解释，不显示内部脚本路径或服务 ID
- 固定组件使用“本地连接 / 指挥家 / 定时任务”，状态统一为“运行中 / 已停止 / 异常”
- “断开”仅用于本地连接，“启动 / 停止 / 重启”用于受管理的后台组件；只刷新 WebView 的操作必须写“刷新界面”
- hover：显露操作按钮（start/stop/logs）
- 展开：日志模态窗（全高 modal）

#### Sidebar Footer

- 默认：只显示“设置”和“运行时管理”两个无文字图标，均通过 Tooltip 提供 L2 说明
- 展开：点击“运行时管理”图标 → 向上弹出 popover panel（`.ga-bridge-panel`）
- Panel 内含运行状态、日志尾和操作按钮；Shell 不再常驻显示状态灯或版本号

#### Tooltip

- 所有 Semi Tooltip 使用统一的紧凑规格：12px 字号、18px 行高、5px × 8px 内边距
- 气泡使用不透明深色表面 `--ui-tooltip-surface`，箭头与气泡同色，确保亮色和暗色界面均有稳定对比度
