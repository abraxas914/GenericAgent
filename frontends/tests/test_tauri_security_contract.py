...
def test_global_tauri_remains_only_because_bundled_compatibility_pages_use_it():
    config = _json(TAURI_ROOT / "tauri.conf.json")
...
def test_e2e_config_only_adds_test_driver_and_dynamic_loopback_connectivity():
    ...
    assert capability["permissions"] == [
        ...
        "allow-capture-macos-window-screenshot",
    ]
...
def test_native_titlebar_metrics_command_is_main_window_only():
    ...
    assert "allow-capture-macos-window-screenshot" not in main["permissions"]
    assert "allow-capture-macos-window-screenshot" not in setup["permissions"]
    assert 'identifier = "allow-capture-macos-window-screenshot"' in permissions
    assert 'commands.allow = ["capture_macos_window_screenshot"]' in permissions
